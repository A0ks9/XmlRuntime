...
class ResourcesPlugin : Plugin<Project> {
        ...
        try {
            ...
            androidComponents.onVariants { variant ->
                ...
                try {
                    ...
                    LOGGER.error("Failed to configure variant ${variant.name}: ${e.message}", e)
                    throw GradleException("Failed to configure variant ${variant.name}", e)
                }
            }
        } catch (e: Exception) {
            LOGGER.error("Failed to apply ResourcesPlugin: ${e.message}", e)
            throw GradleException("Failed to apply ResourcesPlugin", e)
        }
    }
     ...
     * @throws GradleException if task creation fails
     */
    private fun createGenerateTask(
        ...
        try {
            ...
            LOGGER.error("Failed to configure task for variant ${variant.name}: ${e.message}", e)
            throw GradleException("Failed to configure task for variant ${variant.name}", e)
        }
    }
     ...
     * @param project The Gradle project
     * @throws GradleException if configuration fails
     */
